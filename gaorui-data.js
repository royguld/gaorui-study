/* 高睿静态课程数据（从 script.js 抽取，供起始页迁移为标准孩子档案使用） */
(function () {
/* ========== 数据 ========== */
...
/* 归一化：确保每课字段齐全，符合 pod-runtime 的要求 */
Object.keys(subjects).forEach(function (k) {
  (subjects[k].lessons || []).forEach(function (l) {
    if (!Array.isArray(l.steps) || !l.steps.length) l.steps = ["看概念", "看例子", "做小测"];
    if (!Array.isArray(l.narration) || !l.narration.length) l.narration = l.steps.slice();
    if (!l.mistake) l.mistake = "不要只记结论，要能说出原因。";
    if (!Array.isArray(l.terms)) l.terms = [];
    if (!Array.isArray(l.review)) l.review = [];
    if (!Array.isArray(l.quiz)) l.quiz = [];
  });
});
window.GAORUI_SUBJECTS = subjects;
})();
